	...
	m_pSelectionLeader(0)
{
	m_graphicsScene.setContextMenuEventCallback(
		[this] (QGraphicsSceneContextMenuEvent* evt) { return sceneContextMenuEvent(evt); }
	);
}
...
ThumbnailSequence::Impl::invalidateAllThumbnails()
{
	...
	if (m_ptrOrderProvider.get()) {
		m_itemsInOrder.sort(
			[this] (Item const& l, Item const& r) {
				return m_ptrOrderProvider->precedes(l.pageId(), l.incompleteThumbnail, r.pageId(), r.incompleteThumbnail);
			}
		);
	}
...
ThumbnailSequence::Impl::getLabelGroup(PageInfo const& page_info)
{
	...
	return std::unique_ptr<LabelGroup>(new LabelGroup(std::move(normal_text_item), std::move(bold_text_item), std::move(pixmap_item)));
}
